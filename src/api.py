...
from floracorpus.efloraQuery import readEflora, readNames
...
class efloraDescriptions(Resource):
    ...
    def get(self):
        dbparser.replace_argument("family", required=True)
        return readEflora(dbparser.parse_args())

class efloraNames(Resource):

    def get(self, fieldname):
        dbparser.replace_argument("family", required=False)
        return readNames(fieldname, dbparser.parse_args())
...
api.add_resource(efloraNames, '/names/<string:fieldname>')
